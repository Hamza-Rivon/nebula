...
import { affectedConsultantCount, topUnresolvedCluster } from "../derive";
...
export default function GapMap({ data, onSelect, selectedId }: Props) {
  ...
  // Rank clusters by sessionCount * severity → pick top 4 to label inline.
  ...
  return (
    <div className="gapmap-wrap" ref={wrapRef}>
      ...
      >
        ...
        {/* inline labels for top clusters */}
        {bubbles
      ...
      {hover && (
        ...
        >
          ...
          <div className="gapmap-tip-meta">
            ...
            {hover.cluster.sessionCount} sess · {hover.cluster.userCount}{" "}
            consult · sev {hover.cluster.severity.toFixed(2)}
          </div>
        </div>
      )}
    </div>
...
export function GapMapCaption({ data }: CaptionProps) {
  const kicker = topUnresolvedCluster(data.clusters);
  const affected = affectedConsultantCount(data);
  return (
    <div className="gapmap-caption">
      <div className="gapmap-caption-text">
        {kicker ? (
          <>
            <strong>
              {affected} of your {data.users.length} consultants
            </strong>{" "}
            hit the same wall:{" "}
