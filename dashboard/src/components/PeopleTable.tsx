...
// A persona-aware roster. Sortable by any column; click row → drawer.
export default function PeopleTable({
  ...
  const rows = useMemo(() => {
    const rs = data.users.map((u) => ({ user: u, trend: trendByUser.get(u.id) ?? [] }));
    rs.sort((a, b) => {
    ...
    <section className="panel">
      <div className="panel-header">
        <div>
          ...
          <h3 className="panel-title">Consultants</h3>
        </div>
        <div className="panel-meta">
          {data.users.length} tracked · sort by any column
        </div>
      </div>
          ...
          <thead>
            <tr>
              ...
              <th onClick={() => handleSort("name")} className="sortable">
                Consultant <span className="sort-arrow">{arrow("name")}</span>
              </th>
